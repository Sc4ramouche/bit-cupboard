...
def taxicab(a, b):
    ...
    9
    """
    street_diff = abs(street(a) - street(b))
    avenue_diff = abs(avenue(a) - avenue(b))
...
def torque(s):
    assert is_side(s), "torque shall be constructed from the side"
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        assert is_mobile(m), "balanced must be called on mobile"

        torque_l = torque(left(m))
        torque_r = torque(right(m))
        return balanced(end(left(m))) and balanced(end(right(m))) and torque_l == torque_r
...
def totals_tree(m):
    ...
    return tree(left_weight + right_weight, [totals_tree(end(left(m))), totals_tree(end(right(m)))])
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, provided_password):
        ...
        if len(incorrect_attepmts) > 2:
            return "Your account is locked. Attempts: {}".format(incorrect_attepmts)

        if provided_password != password:
            incorrect_attepmts.append(provided_password)
...
def make_joint(withdraw, old_password, new_password):
    ...
    def additional(amount, password):
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert 0 < lower_bound(y) or upper_bound(y) < 0
